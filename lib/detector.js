...
async function patchViteConfig(projectPath, tunnelHost) {
    ...
    const hasTsConfig = await checkFileExists(configTsPath);
    const configFile = hasConfig ? configPath : hasMjsConfig ? configMjsPath : hasTsConfig ? configTsPath : null;
  ...
  server: {
    allowedHosts: true
        ...
        try {
            ...
            console.error('Failed to create vite config:', error.message);
            return false;
        }
    }
    ...
    try {
        ...
        if (content.includes('allowedHosts: true') || content.includes('allowedHosts: "all"') || content.includes("allowedHosts: 'all'")) {
            return true;
        }
        
        if (content.includes('allowedHosts:')) {
            content = content.replace(/allowedHosts:\s*\[([^\]]*)\]/, (match, hosts) => {
                if (tunnelHost && !hosts.includes(tunnelHost)) {
                    return `allowedHosts: ['all', ${hosts}]`;
                }
                return `allowedHosts: ['all']`;
            });
        ...
        console.log('Patched vite config with allowedHosts: all');
        return true;
    } catch (error) {
...
async function ensureAllowedHostsTrue(projectPath) {
    return patchViteConfig(projectPath, null);
}
...
async function getNodeStartCommand(projectPath, port, isStreamlit, tunnelUrl) {
    ...
    let args = ['run', 'dev'];
    const tunnelHost = tunnelUrl ? tunnelUrl.replace('https://', '').replace('http://', '') : null;
    
    if (isNext) {
        args.push('--', '--port', port, '--hostname', '0.0.0.0');
        await patchNextConfig(projectPath, tunnelHost);
    } else if (isVite) {
    ...
    if (tunnelHost) {
        env.VITE_DEV_SERVER_HOSTNAME = tunnelHost;
...
async function patchNextConfig(projectPath, tunnelHost) {
    const configPath = join(projectPath, 'next.config.js');
    const configMjsPath = join(projectPath, 'next.config.mjs');
    const configTsPath = join(projectPath, 'next.config.ts');
    
    const configFile = 
        await checkFileExists(configPath) ? configPath :
        await checkFileExists(configMjsPath) ? configMjsPath :
        await checkFileExists(configTsPath) ? configTsPath : null;
...
const nextConfig = {
    ...
    experimental: {
        allowedRevalidateHeaders: ['x-tunnel-host'],
    }
}

module.exports = nextConfig`;
        await writeFile(join(projectPath, 'next.config.js'), content);
        return true;
    }
    ...
    try {
        ...
        if (content.includes('allowedRevalidateHeaders')) {
            return true;
        }
    ...
    experimental: {
        ...
        if (content.includes('module.exports')) {
            content = content.replace(/module\.exports\s*=\s*\{/, `module.exports = {${insertStr}`);
        } else if (content.includes('export default')) {
            content = content.replace(/export default\s*\{/, `const nextConfig = {${insertStr}\n${content.includes('const nextConfig') ? '' : '}\n\nexport default nextConfig'}`.replace(/const nextConfig = \{[\s\S]*\}\n\nexport default nextConfig/, ''));
        }
        
        await writeFile(configFile, content);
        return true;
    } catch (error) {
        console.error('Failed to patch next config:', error.message);
        return false;
    }
}
...
const LANGUAGE_CONFIGS = {
    nodejs: {
        ...
        start: async (projectPath, port, isStreamlit, tunnelUrl) => getNodeStartCommand(projectPath, port, isStreamlit, tunnelUrl),
        detectPort
    },
    nextjs: {
    ...
    python: {
        ...
        start: (projectPath, port, isStreamlit = false) => {
            ...
            if (shouldDetectStreamlit) {
                cmd = `source venv/bin/activate && streamlit run main.py --server.port ${port} --server.address 0.0.0.0 --server.headless true --server.enableCORS true --server.enableXsrfProtection false`;
            } else {
    ...
    go: {
        ...
        start: (projectPath, port, isStreamlit, tunnelUrl) => ({
            command: 'go',
            args: ['run', 'main.go'],
            cwd: projectPath,
    ...
    static: {
        ...
        start: (projectPath, port, isStreamlit, tunnelUrl) => ({
            command: 'npx',
            args: ['serve', '-l', String(port), '-s', projectPath],
            cwd: projectPath,
