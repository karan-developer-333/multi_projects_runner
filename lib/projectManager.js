...
export async function startProject(projectId, projectPath, languageConfig, isStreamlit = false) {
    ...
    emitProgress(projectId, 'tunnel', 'Creating public URL...', 80);
    ...
    const startConfig = await languageConfig.start(projectPath, port, isStreamlit, tunnelUrl);
...
export function getProjectLogs(projectId, lines = 100) {
    const project = runningProjects.get(projectId);
    if (!project) {
        return { success: false, error: 'Project not found or not running' };
    }
    
    const logs = project.startupOutput || '';
