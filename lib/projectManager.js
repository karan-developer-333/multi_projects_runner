...
async function connectNgrok(port, projectId) {
    ...
    console.log(`[NGROK_AUTH_TOKEN=${authtoken}] found, attempting to create tunnel...`);
    try {
        const ngrokConfig = {
            addr: port,
            proto: 'http',
        ...
        const listner = await ngrok.forward(ngrokConfig);
        console.log(`[${projectId}] Ngrok tunnel established at ${listner.url()}`);
        return listner.url();
    } catch (error) {
        console.error(`[${projectId}] Ngrok tunnel error:`, error.message);
        ...
        try {
            ...
            console.error(`[${projectId}] :`, fallbackError.message);
            return null;
        }
    }
}

async function disconnectNgrok(url) {
    try {
        if (url) {
...
export async function stopProject(projectId) {
    ...
    try {
        if (project.ngrokUrl) {
            await disconnectNgrok(project.ngrokUrl);
        }
